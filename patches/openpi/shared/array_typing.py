...
# runtime type-checking decorator — no-op'd (RLinf perf patch). The jaxtyped +
# beartype validation costs ~3ms/predict of pure CPU on the torch rollout path
# (in JAX it is traced away by jit; the torch port pays it on every call).
# Must be disabled here at the definition site: guards in downstream modules
# run too late because dataconfig imports openpi.models.model first.
def typecheck(t: T) -> T:
    return t
